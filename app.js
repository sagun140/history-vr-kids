(() => {
  ...
  // The time scale, the era windows and the chain search live in threads.js,
  // shared with the VR version so the two can never disagree about history.
  const { byId, out, inn, whyOf, ANCHORS, toPos, toYear, fmtYear, when } = window.THREADS;
  ...
  // ----- Eras and the chains between them (both from threads.js) -----
  const range = p => [toYear(Math.max(0, p - state.w / 2)), toYear(Math.min(1, p + state.w / 2))];
  const eras = () => window.THREADS.eras(state);
  const findChains = () => window.THREADS.findChains(state, 5);
  ...
  // ----- Shareable links: every view has an address -----
  function writeURL() {
    const p = new URLSearchParams();
    if (state.focus) p.set("event", state.focus);
    else { p.set("then", toYear(state.a)); p.set("now", toYear(state.b)); if (state.w !== 0.05) p.set("era", state.w); }
    history.replaceState(null, "", location.pathname + "?" + p);
  }
  function readURL() {
    const p = new URLSearchParams(location.search);
    if (p.has("era")) { const w = +p.get("era"); if (w >= 0.02 && w <= 0.2) { state.w = w; $("size").value = w; } }
    if (p.has("then") && p.has("now")) { state.a = toPos(+p.get("then")); state.b = toPos(+p.get("now")); update(); }
    else presets[0].click();
    if (p.has("event") && byId.has(p.get("event"))) openEvent(p.get("event"));
    return p.has("then") || p.has("event");
  }
